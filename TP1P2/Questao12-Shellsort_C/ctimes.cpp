...
void shellSort(Time** times, int tam, int* movimentacoes, int* comparacoes);
...
{   
	...
	log = fopen("./651047_shellsort.txt", "w");
    do
    {
	...
	shellSort(times, index, &movimentacoes, &comparacoes);
...
{	
	...
	while(espaco > 0)
	{
		for(int i = espaco; i < tam;i++)
		{
			...
			*comparacoes += 1;
			while((j > espaco-1) && (strcmp(tmp->tecnico, times[j-espaco]->tecnico) < 0))
			{
				*comparacoes += 1;
				*movimentacoes += 1;

				times[j] = times[j-espaco];
				j -= espaco;
			}
			*movimentacoes += 1;
			times[j] = tmp;

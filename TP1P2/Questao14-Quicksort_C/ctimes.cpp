...
void quickSort(Time** times, int* movimentacoes, int* comparacoes, int esq, int dir);
...
{   
	...
	log = fopen("./651047_quicksort.txt", "w");
    do
    {
	...
	quickSort(times, &movimentacoes, &comparacoes, 0, index-1);
	quickSort(times, &movimentacoes, &comparacoes, 0, index-1);
...
{	
	...
	do{
		...
		*comparacoes += 2;
		...
		while(strcmp(times[i]->apelidos, times[pivo]->apelidos) < 0)
		{
			*comparacoes += 1;
			i++;
		}
		while(strcmp(times[j]->apelidos, times[pivo]->apelidos) > 0)
		{
			*comparacoes += 1;
			j--;
		}
	...
	if(j > esq)
		quickSort(times, movimentacoes, comparacoes, esq, j);

	if(i < dir)
		quickSort(times, movimentacoes, comparacoes, i, dir);

...
void bubbleSort(Time** times, int tam, int* movimentacoes, int* comparacoes);
...
{   
	...
	bubbleSort(times, index, &movimentacoes, &comparacoes);
...
void bubbleSort(Time** times, int tam, int* movimentacoes, int* comparacoes)
{
	...
	while(!ordenado)
	{
		...
		{	
			*comparacoes += 1;
			if(strcmp(times[i]->liga, times[i+1]->liga) > 0)
			{
				Time* aux      =  times[i+1];
				times[i+1]     =  times[i];
				times[i]       =  aux;
...
/*
	printf("DataString = '%s'\n", dataString);
	printf("Purinho: %d = data[0], %d = data[1], %d = data[2]\n\n", data[0], data[1], data[2]);
*/	

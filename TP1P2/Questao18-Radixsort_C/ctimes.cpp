...
void radixSort(Time** times, int tam, int* movimentacoes, int* comparacoes);
...
{   
	...
	log = fopen("./651047_radixsort.txt", "w");
    do
    {
	...
	radixSort(times, index, &movimentacoes, &comparacoes);
...
{	
	...
	b = (Time**)calloc(tam, sizeof(Time));
	
	for(int i = 0; i < tam; i++)
		if( times[i]->paginaTam > maior->paginaTam)
			maior = times[i];

	while(maior->paginaTam/exp > 0)
	{
		int balde[10] = { 0 };
		for(int i = 0; i < tam; i++)
		{
			balde[(times[i]->paginaTam/exp) % 10]++;
		}
		...
		{
			b[--balde[(times[i]->paginaTam / exp) % 10 ]] = times[i]; 
		}
    ...
    long int tamPag;
   		    ...
   		    while(!feof(arq))
		    {
			    ...
			    if(isSubstring(linha, "infobox vcard"))
			    {
					...
					// Alguns tecnicos estao com ' ' na frente do nome
					while(tecnico[0] == ' ')
					{

...
void insertionSort(Time** times, int* movimentacoes, int* comparacoes, int tam);
...
{   
	...
	insertionSort(times, &movimentacoes, &comparacoes, index);
...
{	
	...
	for(int i = 0; i < 10; i++)
	{
		...
		*comparacoes += 1;
		for(j = i+1; j < tam; j++)
		{	
			if(times[j]->fundacaoAno < times[menor]->fundacaoAno) 
			{	
				menor = j;
			}
		}

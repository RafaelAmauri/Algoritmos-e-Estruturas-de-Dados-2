...
void inserirPosicao(Time** times, char instrucao[100], int cabeca, int fim);

void removerPosicao(Time** times, int fim);
...
{   
    ...
    char nomeArquivo[100];
    bool notFim = 1;
	int cabeca = 0, fim = 0, soma_anos, quantElementos;
    ...
    do
    {
		...
		if(notFim)
        {
			...
			if((cabeca-fim) == 1)
				removerPosicao(times, cabeca);
			
			inserirPosicao(times, nomeArquivo, cabeca, fim);
			fim = ++fim % 5;
			

			for(int i = cabeca % 5; i != fim; i = (i+1) % 5)
			{
			...
			if(cabeca-fim == 1)
			{
			soma_anos+= times[fim%5]->fundacaoAno;
			quantElementos++;
			}

			printf("%.0f\n", (float)soma_anos/(quantElementos));
	...
	for(int i = 0; i < quantInstrucoes; i++)
	{
		...
		if(isSubstring(instrucao, "I"))
		{
			...
			inserirPosicao(times, instrucao, cabeca, fim+1);
			fim = ++fim % 5;
...
{	
	...
	times[fim % 5] = ler(nomeTime, times[fim%5]);
}

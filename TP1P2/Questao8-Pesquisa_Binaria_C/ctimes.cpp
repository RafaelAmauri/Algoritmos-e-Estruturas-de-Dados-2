...
/*
 * Aloca espaco na memoria para os times;
 */
void criar(Time **times, int quant);


/*
 * Pesquisa binaria em um array ordenado de times
 */
bool pesquisa(Time **times, char nomeTime[], int quant);
...
{   
	...
	log = fopen("./651047_binaria.txt", "w");
	...
	*/	
	...
	do
	{
		...
		if(notFim)
		{
			if(pesquisa(times, nomeTime, index))
				printf("SIM\n");
...
bool pesquisa(Time **times, char nomeTime[], int quant)
{
	int pos_maior = quant;
	int pos_menor = 0;
	int pos       =  (pos_maior+pos_menor)/2;
	bool flag     =  false;
	
	while( (pos >= 0) && (pos <= quant) && !flag)
	{
		COMPARACOES++;
		if(strcmp(nomeTime, times[pos]->nome) == 0)
		{
			pos   =  quant+1;
			flag  =  true;
		}
		...
		if( (pos_maior+pos_menor)/2 == pos)
		{
			pos = quant+1;
			continue;
		}	
		
		pos = (pos_maior+pos_menor)/2;
	}
...
void criar(Time **times, int quant)
{
	for(int i = 0; i < quant; i++)
		times[i] = (Time*)malloc(sizeof(Time));
}

...
/*
 * Funcao hash que segue a formula (time->tamPag % tamanho_da_tabela)
 */
int hash(Time *elemento);


/*
 * Insere um time na tabela hash usando a funcao hash()
 */
void inserir(No tabela[], Time *elemento);


/*
 * Seta todos Nos da tabela para NULL, para nao pegar lixo da memoria
 */
void criar(No tabela[]);

/*
 * Pesquisa se um time existe ou nao na tabela; Imprime "SIM" ou "NÃO" na tela
 */
void pesquisar(No tabela[], char nomeTime[]);


/*
 * Mostra todos elementos da tabela
 */
void mostrar(No tabela[]);
...
{   
	...
	log = fopen("./651047_hashIndireta.txt", "w");
...
void pesquisar(No tabela[], char nomeTime[])
{
	...
	{
		COMPARACOES++;
		if(tabela[i].time != NULL)
		{
			if(strcmp(tabela[i].time->nome, nomeTime) == 0)
			{
				printf("%s SIM\n", nomeTime);
				flag = true;
			}
			...
			else
			{
				...
				{
					COMPARACOES++;
					if(strcmp(prox->time->nome, nomeTime) == 0)
					{
						printf("%s SIM\n", nomeTime);
						flag = true;
					}
...
void inserir(No tabela[], Time *elemento)
{
	...
	COMPARACOES++;
	if(tabela[pos].time == NULL)
		tabela[pos].time = clone;

	else if(tabela[pos].prox == NULL)
	{
		COMPARACOES++;
		tabela[pos].prox        =  (No*)malloc(sizeof(No));
		tabela[pos].prox->time  =  clone;
		tabela[pos].prox->prox  =  NULL;
	}	

	else
	{
		No* prox = tabela[pos].prox;	
...
int hash(Time *elemento)
{
	return elemento->fundacaoAno % TAMANHO;
}
